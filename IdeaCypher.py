...
def binary_plaintext_to_64bit_array(plain_text):
    binary_buffer = []
    for index in range(15,len(plain_text),16):
        binary_buffer.append(plain_text[index-15:index+1])
    return binary_buffer

def key_schedule(key):
    #b64_key = str_to_b64(key)
    b64_key = key_validation(key)
    print(key, len(key))
    ...
    while len(keys)<28:
        print("Round", ins)
        for index in range(3,len(binary_key),4):
            print(binary_key[index-3:index+1])
            keys.append(binary_key[index-3:index+1])
        binary_key = binary_key[6:len(binary_key)] + binary_key[0:6]
        ins = ins + 1
    return keys
...
def cypher_process(block,keys):
    entries = [block[0:4],block[4:8],block[8:12],block[12:16]]
    print(entries)
    for x in range (0,4):
...
def encrypt(text,keys):
    ...
    #print("text to b64  ", text_b64)
    ascii_buffer = to_ascii_code(text_b64) 
    #print("ascii  ", ascii_buffer )
    binary_buffer = buffer_to_binary(ascii_buffer)
    print("binary buffer  ", binary_buffer)
...
def decription_keys_scheduling(encription_keys):
    encription_keys = encription_keys[0:28]
    decription_keys = []
    ...
    for key_index in range(0, 3):
        ...
        # 48,49,50,51,     46,47,
    #half round
    decription_keys.append(get_binary_16(multiply_inverse(get_str_from_binary(prepared_encription_keys[18]))))
    decription_keys.append(get_binary_16(additive_inverse(get_str_from_binary(prepared_encription_keys[19]))))
    decription_keys.append(get_binary_16(additive_inverse(get_str_from_binary(prepared_encription_keys[20]))))
    decription_keys.append(get_binary_16(multiply_inverse(get_str_from_binary(prepared_encription_keys[21]))))
...
def decrypt(text, keys):
    ...
    #print("ascii_bugffer  ", ascii_buffer)
    binary_buffer = buffer_to_binary(ascii_buffer)
...
def idea_cypher():
    ...
    print(decription_keys)
    print(len(decription_keys))
    print(test_decription_keys(decription_keys))
    crypted = encrypt(text, keys)
    print(test_encript(crypted))
    #print(crypted)
    #print("asd",chr(156),chr(172))
    #print([ord("Ü"),ord("o"),ord("?"),ord("Y")])
    #print (buffer_to_binary([ord("Ü"),ord("o"),ord("?"),ord("Y")]))
    decripted = decrypt(crypted, decription_keys)
    print(decripted)
    print(test_decript(decripted))

def test_keys(keys):
    keys = keys[0:28]
    test_keys = ["1101","1100","0110","1111","0011","1111","0101","1001","0001","1011","1100","1111","1101",
    "0110","0111","0111","1111","0011","1111","0101","1001","1101","1100","0110","1111","1101","0110","0111"]
    for i in range(0,28):
...
def test_decription_keys(keys):
    ...
    "1010","1001","0101","1100","1111","0111","0111","1111","1110","0011","1111","0100","0100","1010","1000"]
    for i in range(0,28):
        if keys[i] != test_keys[i]:
            return False
    return True
